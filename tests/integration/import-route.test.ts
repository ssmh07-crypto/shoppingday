...
vi.mock('@/lib/auth/admin', () => ({ AuthenticationError: MockAuthenticationError, requireAdmin }))
...
describe('친구도매 import Route Handler', () => {
  beforeEach(() => { requireAdmin.mockReset(); importByExternalId.mockReset() })

  it('인증되지 않은 관리자 접근을 차단한다', async () => {
    requireAdmin.mockRejectedValue(new MockAuthenticationError('관리자 로그인이 필요합니다.'))
    ...
    expect(await response.json()).toMatchObject({ success: false, error: { code: 'authentication_error' } })
  ...
  it('잘못된 goodsno를 서비스 호출 전에 거부한다', async () => {
    ...
    const response = await POST(new Request('http://localhost/api/suppliers/dome/products/import', {
      method: 'POST', body: JSON.stringify({ goodsno: '../secret' }), headers: { 'content-type': 'application/json' },
    }))
