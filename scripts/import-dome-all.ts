...
async function main() {
  if (!process.argv.includes('--confirm')) {
    throw new Error('실제 친구도매 API와 Supabase DB를 사용하려면 --confirm을 붙여 주세요.')
  }
  ...
  const result = await createDomeImportService().importAll(admin.userId, (progress) => {
    console.info(
  ...
  console.info(
    `완료: 총 ${result.total}개, 신규 ${result.created}개, 갱신 ${result.updated}개`,
  )
}

void main()
  .catch((error) => {
    console.error(error instanceof Error ? error.message : '전체 상품 가져오기에 실패했습니다.')
    process.exitCode = 1
  })
  .finally(closeDb)
