...
네이버 v2 등록 payload는 저장 모델 자체가 아니라 `naver-product-payload.ts`에서 생성한다. 이 변환기는 공급처 상품번호를 판매자 관리 코드로 사용하고 카테고리, 네이버 속성, 조합 옵션, 태그를 API 구조에 맞게 변환한다. 옵션 재고 합계를 원상품 재고로 사용하며, 옵션 없는 상품은 별도의 단일 재고가 필요하다. payload hash는 객체 키 순서와 무관한 SHA-256으로 생성해 향후 중복 발행 방지에 사용한다.

외부 이미지 URL은 네이버 상품 등록 본문에 직접 넣지 않는다. 활성 이미지마다 네이버 이미지 업로드 API가 반환한 `storedUrl`이 있어야 한다. 배송, A/S, 원산지, 상품정보제공고시, 부가가치세 유형, 미성년자 구매 가능 여부와 채널 노출 정책도 명시되지 않으면 payload 생성을 차단한다.
