...
- 선택한 스마트스토어 연결로 현재 채널 상품 전체 정보를 다시 조회한 뒤 상품명과 판매자 검색 태그만 교체하여 기존 원상품 수정 API에 반영
- 사용자가 같은 검색 조건에서 확인한 실제 키워드 순위를 날짜별 이력으로 저장

이번 MVP에서 구현하지 않음:

- 다른 판매자의 상품이나 공개 페이지를 HTML 크롤링해 내용을 수집
- 생성한 상품명을 네이버 상품에 자동 반영
- 판매자센터 키워드 분석·상품 순위 자동 조회, 로그인 자동화, HTML 크롤링

네이버 상품 수정은 기존 커머스 adapter를 사용하며 사용자의 별도 확인 뒤에만 호출한다. 네이버 응답이 성공한 뒤에만 현재 상품명과 태그를 갱신하고, 성장 관리의 로컬 저장 성공을 네이버 반영 성공처럼 표시하지 않는다.

네이버 Developers의 쇼핑 검색 Open API는 2026년 7월 31일 종료 예정이므로 신규 자동 순위 수집 기반으로 사용하지 않는다.
<https://developers.naver.com/notice/article/32564>
공식적으로 지속 가능한 대체 순위 데이터 공급원이 확인되기 전에는 사용자가 확인한
실제 순위를 직접 기록하거나, 사용자가 요청한 때에만 로컬 Chrome 확장 프로그램이
가격비교 PC 공개 화면에서 최대 100위까지 관측한다. 이는 공식 API 순위가 아니며
추정 순위를 만들지 않는다.

2026-07-30 확인한 공개 커머스 API의 상품·주문·정산·문의·판매자정보 범위에는 상품별
클릭을 발생시킨 유입 검색어를 조회하는 API가 없다. 판매자센터 화면에서 확인 가능한
통계가 있더라도 이를 커머스 API 값으로 간주하거나 비공개 화면을 자동 수집하지 않는다.
...
- 대상: `https://zicgam.com/product/detail.html?product_no={number}` 형식의 상품 한 건
- 실행 주체: 사용자가 상품별 확인 또는 전체 확인을 직접 누른 경우에만 로그인된
  로컬 Chrome 확장 프로그램
- 판별 상태: 판매 가능, 옵션 일부 품절, 전체 품절, 단종, 로그인 필요, 판별 불가
- 저장 데이터: 확인 시각, 상품 URL, 상태, 화면에서 확인한 근거와 옵션명
- 저장하지 않는 데이터: 직감 계정, 비밀번호, 로그인 쿠키, 페이지 전체 HTML
- 안전장치: 직감 도메인과 숫자 `product_no`만 허용하고 결과로 원격 판매 상태를
  자동 변경하지 않는다.
- 전체 확인은 탭 하나에서 한 상품씩 순서대로 처리하고 상품 사이에 3~5초 간격을
  둔다. 결과를 한 건씩 즉시 저장하며 로그인 필요 상태가 나오면 작업을 중단한다.

직감의 예시 상품 `product_no=3649`는 비로그인 요청 시 승인회원만 구매할 수 있다는
안내와 로그인 이동만 반환했다. 따라서 실제 품절·단종 상태는 승인된 직감 계정으로
로그인한 Chrome에서 확인해야 하며, 공개 요청 결과를 상품 상태로 간주하지 않는다.
