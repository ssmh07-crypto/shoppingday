...
- 중계 서버는 코드에 명시한 카테고리·상품 메타데이터·판매자 배송정보 조회,
  이미지 업로드와 상품 등록·수정·상태 변경·삭제 경로만 제공하며 임의 URL·메서드·헤더 전달을 허용하지
  않는다.
...
경로에 적용한다. 쓰기 요청은 이미지 업로드와 상품 등록, 숫자형 상품번호를 사용하는
원상품 수정·판매 상태 변경·채널 상품 삭제·원상품 삭제만 허용한다. 카테고리 메타데이터 경로는 숫자형 `categoryId` 쿼리만 전달하며
