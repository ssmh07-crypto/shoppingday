...
상세 조회와 API 저장은 각각 45초 시간 제한을 두며, 처리 시작·상품 시작·저장 완료 이벤트로
현재 처리 번호와 마지막 응답 시각을 관리 화면에 전달한다.
상품 단위 상세 조회와 저장 실패는 1회 재시도하고, 연속 실패로 중단할 때 최근 실패의
상품번호·처리 단계·오류 메시지를 보존해 공급처 판독과 앱 저장 문제를 구분한다.
직감 저장은 외부 상품번호로 멱등 처리되므로 일시적인 DB 연결 오류에 한해 요청 단위로
새 연결을 사용해 한 번 재시도한다. API가 아닌 플랫폼 오류 응답은 HTTP 상태와 응답 형식을
확장 프로그램에 전달해 애플리케이션 오류와 구분한다.
HTTP 429·5xx는 개별 상품 오류로 집계하지 않고 동일 요청을 5초·15초·30초·60초 지수
백오프로 재시도한다. 백오프 이후에도 실패하면 플랫폼 장애로 전체 작업을 중단해 수천 개
상품을 잘못된 실패로 기록하거나 장애 중 요청을 계속 보내지 않는다.
확장 프로그램은 작업 탭 ID, 최신 진행 이벤트와 누적 저장 결과를 세션 저장소에 유지한다.
Shoppingday의 다른 관리자 화면으로 이동하는 동안에도 같은 탭의 브리지가 저장 요청을
처리하며, 가져오기 화면으로 돌아오면 저장된 상태를 조회해 진행 UI를 복구한다.
