...
2026-09-08 최종 범위에서는 위탁상품 전체의 공급처 변경 관리와 선택된 성장상품의
정밀 최적화를 구분한다. 아래 Chrome 재고 확인은 기존 성장상품 기능에 대한 설명이며,
전체 위탁상품의 변경 반영 완료를 의미하지 않는다. 구현 단계와 남은 작업은
`implementation-roadmap-2026-09-08.md`를 따른다.

친구도매 변경 수집 예약은 `supplier_sync_schedules`에 저장한다. 설정 API는 관리자
인증과 소유자 조건을 확인하며, Actions의 예약 실행기는 관리자 역할을 다시 확인하고
행 잠금으로 기한이 지난 예약을 선점한다. 기존 `supplier_sync_jobs`와 동기화 실행기를
재사용한다. 이 예약은 공급처 원본 수집만 수행하며 스마트스토어 자동 변경은 포함하지 않는다.

위탁상품의 성장관리 연결 API는 현재 사용자의 상품 접근 권한과 발행 대상 스토어를
확인한 뒤 해당 스토어에 등록된 상품번호로 기존 키워드 관리 생성 서비스를 호출한다.
같은 상품번호의 기존 성장관리 기록은 그대로 열어 변경 이력을 유지한다.
...
평소 변경 확인은 스마트스토어에 등록 연결된 공급처 상품의 상세주소만 Chrome 확장에
전달해 순회한다. 서버와 Actions 저장 단계에서도 등록 연결을 다시 확인하며, 연결이 없는
상품은 변경 작업 중 새로 생성하지 않는다.

등록상품관리 화면은 활성 `product_publications`를 기준으로 기본 목록을 만들고 기본 공급처
연결과 가격·상태·상세 반영 대기열을 결합한다. 행별·선택 상품 작업은 상품 ID 범위를 API에
전달하고, 조회 조건 전체 작업은 도매처와 변경 종류만 전달해 페이지네이션 밖의 대상도
처리한다. 실제 외부 반영 서비스는 owner와 최신 공급처·판매자 편집·원격 상품번호를 다시
검증한다.

전체상품 탐색은 사용자가 최초 연결·새 상품 후보 수집·누락 복구를 선택할 때만 실행한다.
홈에서 확인한 `전체상품` 목록 하나만 사용한다. 목록 URL의 `page` 값을
