...
- 네이버 카테고리 필수속성 조회는 서버 오류일 때 한 번 자동 재시도하며 실패 상태에서
  사용자가 다시 불러올 수 있다.
- 스마트스토어 배송정책은 JSON 입력을 제거하고 네이버 API에서 출고지·반품교환지,
  묶음배송 그룹과 계약된 반품 택배사를 불러오는 선택형 폼으로 변경했다. 배송비·지역
  추가비·반품비·교환비는 사용자 기본 정책으로 저장하고 상품별 덮어쓰기를 유지한다.
...
- 상품 편집의 배송정책이 채널 기본값을 상속하는 경우에도 네이버 출고지·반품지·반품
  택배사·묶음배송 그룹 조회와 선택 컨트롤은 활성화한다. 값을 선택하면 해당
  `deliveryInfo`가 자동으로 상품별 설정으로 전환되며, 불러온 항목 수를 화면에
  표시한다.
- 회귀 테스트: 실제 다수 후보에서도 두 명사 기준점 사이의 특징이 누락되지 않고,
  기본 명사 `욕실화`가 최대 두 번이며 최종 제목이 50자를 넘지 않는지 확인한다.
- TypeScript 통과
- ESLint 통과
- 전체 테스트: 43개 파일, 219개 통과
- Next.js 프로덕션 빌드 통과
