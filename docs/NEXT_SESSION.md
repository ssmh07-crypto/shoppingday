...
- Chrome 확장 프로그램을 0.4.1로 올리고 `위탁상품 가져오기` 화면에
...
- 사용자가 입력한 직감 표시 전체 상품 수와 발견 수가 정확히 같지 않으면 상세 조회와
  DB 저장을 시작하지 않는 안전장치를 추가했다.
...
1. `chrome://extensions`에서 `Shoppingday 순위·공급처 상태 확인` 0.4.1을 다시 로드한다.
