...
- 2026-08-15에 HTTP 429로 실패했던 스마트스토어 상품은 릴레이의 판매자별 클라이언트·
  ...
  성공했다고 사용자가 확인했다.
- Chrome 확장 프로그램을 0.5.8로 올렸다. 소싱 조사 `연관 키워드 분류`에서 사용자가
...
- PC 가격비교 1페이지의 자연검색 상품 카드 최대 40개를 기준으로 광고와 중복을 제외하고
  ...
  반환한다.
- 상품명 추천 기준은 기본 30%이며 화면에서 1~100%로 조정할 수 있다. 상품명 기준 미달
  후 부가정보, 카테고리 순으로 추천하고 어디에도 없으면 태그 후보로 제안한다. 태그는
  실제 판매자 입력 태그를 확인한 사실이 아니라 운영상 추론이라고 명시한다.
- 개별 분석 결과는 자동 저장하거나 분류를 덮어쓰지 않는다. 사용자가 `추천 적용`을 누른
  뒤 소싱 조사를 저장해야 반영된다. CAPTCHA·접근 제한 또는 상품 카드 식별 실패 시
  중단한다.
- `전체 다시 분류`는 기존 분류를 포함한 연관키워드 전체를 순차 재분석해 추천 위치를
...
- 검증: Vitest 61개 파일·341개 테스트, TypeScript, ESLint, 확장 프로그램 JavaScript
  문법·manifest JSON, Next.js production build 통과.
