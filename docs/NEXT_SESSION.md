...
- 최신 구현은 확장 프로그램 0.5.0의 하이브리드 방식이다. Chrome은 승인회원 로그인으로
...
- Chrome 확장 프로그램을 0.5.0으로 올리고 `위탁상품 가져오기` 화면에
...
2. `chrome://extensions`에서 `Shoppingday 순위·공급처 상태 확인` 0.5.0을 다시 로드한다.
