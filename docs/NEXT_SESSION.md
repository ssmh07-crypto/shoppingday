...
- Chrome 확장 프로그램을 0.4.3으로 올리고 `위탁상품 가져오기` 화면에
  `직감 전체 상품 가져오기`를 추가했다.
- 확장 프로그램은 로그인된 직감 승인 계정으로 홈·카테고리·페이지를 탐색해
...
- 상품 수는 신규·단종에 따라 변하므로 고정값이나 사용자 입력값과 일치하도록 강제하지
  ...
  상세 조회와 DB 저장을 시작한다.
- 0.4.2에서도 모든 카테고리를 각각 순회해 직감의 약 50개 전체상품 페이지 대신
  108페이지와 2,122개 상품을 발견했다. 0.4.3부터 홈의 `전체상품` 링크 하나만 선택하고
...
1. `chrome://extensions`에서 `Shoppingday 순위·공급처 상태 확인` 0.4.3을 다시 로드한다.
