...
- Chrome 확장 프로그램 0.5.17에 이불삼촌 호스트 권한을 추가하고 기존 Cafe24 목록·상세
...
- 공개 페이지 표본에서는 전체상품 152개와 다음 빈 페이지를 확인했다. 실제 로그인 가격 DOM과
  전체 저장은 사용자가 확장 프로그램을 다시 로드한 뒤 실행해 확인해야 한다.

### 다음 세션 우선 확인

1. `chrome://extensions`에서 Shoppingday 확장 프로그램 0.5.17을 다시 로드하고 Shoppingday
   가져오기 페이지도 새로고침한다.
2. 이불삼촌 로그인 상태에서 `신규·변경 상품 확인`을 실행해 발견 수, 첫 상품의 숫자 공급가,
   옵션과 이미지가 정상 판독되는지 확인한다.
3. GitHub Actions `이불삼촌 상품 가져오기`가 실행되고 신규·갱신·변경 없음 합계가 전체 저장
   수와 맞는지 확인한다. 가격 판독 실패 시 재시도하기 전에 실제 로그인 등급과 상품 HTML을
   확인한다.
