...
function decodeXml(buffer: Buffer, contentType: string): string {
  ...
  const charset =
    contentType.match(/charset=([^;\s]+)/i)?.[1] ?? declared ?? "utf-8";
  return iconv.decode(
    buffer,
    /euc-?kr|ks_c_5601-1987/i.test(charset) ? "euc-kr" : "utf-8",
  );
}
...
export class LiveDomeClient implements DomeClient {
  ...
  async fetchProduct(
    ...
    if (query?.runout !== undefined) body.set("runout", String(query.runout));
    if (query?.opened) {
      body.set("opendate_s", query.opened.from);
      body.set("opendate_e", query.opened.to);
    }
    if (query?.modified) {
      body.set("modidate_s", query.modified.from);
      body.set("modidate_e", query.modified.to);
    }
    ...
    try {
      ...
      const buffer = await readLimitedBody(
        response,
        this.env.DOME_API_MAX_RESPONSE_BYTES,
      );
      if (buffer.length === 0) {
        throw new SupplierError(
      ...
      const diagnostic = toSafeDiagnostic(error, [
        this.env.DOME_API_ID,
        this.env.DOME_API_KEY,
      ]);
...
export const domeClientInternals = {
  readLimitedBody,
  decodeXml,
  toSafeDiagnostic,
};
