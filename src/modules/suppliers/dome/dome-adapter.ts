...
export class DomeAdapter implements SupplierAdapter {
  ...
  async fetchProduct(goodsno: string): Promise<SupplierFetchResult> {
    ...
    const exact = products.filter(
      (product) => product.externalProductId === goodsno,
    );
    if (exact.length === 0) {
      throw new SupplierError(
        "supplier_product_not_found",
        "해당 친구도매 상품을 찾지 못했습니다.",
      );
    }
  ...
  async fetchProducts(
    ...
    const response = await this.client.fetchProduct(undefined, query);
    const envelope = parseDomeXml(response.xml);
    return {
      products: envelope.products.map(mapDomeProduct),
