...
export const ebulsamchonCapturedProductSchema = z.object({
  externalProductId: z.string().regex(/^\d+$/).max(30),
  url: httpUrl.refine(
    (value) => new URL(value).hostname === "xn--wr3bq2dn2hzng.com",
  ),
...
export type EbulsamchonCapturedProduct = z.infer<
  typeof ebulsamchonCapturedProductSchema
>;

export class EbulsamchonImportService {
...
function toSupplierProduct(input: EbulsamchonCapturedProduct): SupplierProduct {
  return {
    supplierCode: "ebulsamchon",
