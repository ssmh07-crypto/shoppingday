...
function query(params: ProductListSearchParams, page: number) {
  const queryString = productListQuery(params, ["page", "edit"]);
  queryString.set("page", String(page));
  return `/admin/products?${queryString}`;
}

function editorHref(params: ProductListSearchParams, id: string) {
  const queryString = productListQuery(params, ["edit"]);
...
function productListQuery(
  ...
  for (const [key, value] of Object.entries(params)) {
    if (value && !excludedKeys.includes(key)) queryString.set(key, value);
  }
  return queryString;
}
