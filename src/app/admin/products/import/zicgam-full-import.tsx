...
export function ZicgamFullImport() {
  ...
  const extensionReady = extension.available && isMinimumVersion(extension.version, "0.5.2");
    ...
    <section className="card" aria-live="polite">
      ...
      <p className={`notice${extensionReady ? "" : " error"}`}>
        {extensionReady
          ...
          : `Chrome 확장 프로그램 0.5.2 이상이 필요합니다${extension.version ? ` (현재 ${extension.version})` : ""}. 확장을 다시 로드하고 이 페이지를 강력 새로고침해 주세요.`}
      </p>
      {phase === "discovering" && (
        <p className="notice">
