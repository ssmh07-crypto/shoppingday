  ...
  return (
    <main className="container wide">
      <div className="page-head">
        <div>
          <h1>내 상품</h1>
          <p>저장된 DB 정보만 조회하며 공급사 API를 호출하지 않습니다.</p>
        </div>
      ...
      <form className="toolbar">
        <input
          ...
          placeholder="판매용/원본 상품명, 공급처 상품번호 검색"
        />
        <select name="filter" defaultValue={params.filter ?? ""}>
          ...
          <option value="missing_price">가격 미입력</option>
        ...
        <table>
          ...
          <tbody>
            {result.items.map((item) => {
              const image =
                item.selectedImages.find((i) => i.enabled && i.isPrimary) ??
                item.selectedImages.find((i) => i.enabled);
              return (
                <tr key={item.id}>
                  <td>
                    {image ? (
                  ...
                  <td>
                    <Link href={`/admin/products/${item.id}/edit`}>
                      <strong>{item.title || "(상품명 미입력)"}</strong>
                    </Link>
                    <small>
                      {item.originalName}
                      <br />
                  ...
                  <td>
                    {item.supplierPrice ?? "-"} /{" "}
                    {item.sellingPrice?.toLocaleString() ?? "-"} KRW
                  </td>
      ...
      {!result.items.length && (
        <p className="card">조건에 맞는 상품이 없습니다.</p>
      )}
      <nav className="pagination">
        {result.page > 1 && (
          <Link href={query(params, result.page - 1)}>이전</Link>
        )}
        <span>
          {result.page} /{" "}
        ...
        {result.page * result.pageSize < result.total && (
          <Link href={query(params, result.page + 1)}>다음</Link>
        )}
      </nav>
    </main>
  );
...
function query(params: Record<string, string | undefined>, page: number) {
